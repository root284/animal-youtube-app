...
SCRIPT_SYSTEM = (
    ...
    "【오프닝 구조 원칙】\n"
    "1. 시청자가 이미 동의할 상식을 먼저 확인시킨다.\n"
    "2. '그런데 이게 어떤 상황에서는 완전히 틀렸다면?'으로 연결한다.\n"
    "3. 세 번째 문장 안에 최대 스케일(생태계·지구·인류)을 배치한다.\n"
    "4. 본론은 역사적 맥락 또는 지리적 배경으로 시작해 신뢰를 구축한다.\n\n"
    "【마무리 구조 원칙】\n"
    "- 해결책이 없거나 불완전한 경우 솔직히 인정한다.\n"
    "- '동물의 문제가 아니라 인간의 문제'로 귀결 가능한지 검토한다.\n"
    "- 철학적 여운이 있는 한 문장으로 끝맺는다."
)
...
def api_search_items():
    try:
        ...
        system = (
            ...
            "아이템 선정 기준 (4가지 모두 충족할수록 좋음):\n"
            "1. 일상 접점: 대부분의 시청자가 알고 있는 동물인가?\n"
            "2. 상식 존재: 이 동물에 대해 널리 알려진 믿음이 있는가?\n"
            "3. 반전 가능: 그 믿음이 조건부로 또는 완전히 틀렸는가?\n"
            "4. 스케일 확장: 개인 → 생태계 → 지구/인류로 연결 가능한가?\n\n"
            "최신 뉴스가 없어도 위 4가지 기준을 충족하면 선정 가능."
        )
        prompt = (
            f"웹 검색으로 동물 관련 소재 중 위 4가지 기준을 최대한 충족하는 "
            f"한국 유튜브 콘텐츠 아이템 5개 추천. 최신 뉴스 없어도 무방.{direction_line}{exclude_line}\n\n"
            "JSON 배열만 반환:\n"
...
def api_generate_script():
    try:
        ...
        prompt = (
            ...
            "[인트로] → 시청자가 동의할 상식 확인 → 반전 질문('그런데 이게 틀렸다면?') → 최대 스케일(생태계/지구/인류) 언급\n"
            "[본문] → 역사적 맥락 또는 지리적 배경으로 시작 → 수치·에피소드·확인된 사실 활용\n"
            "[동물 인터뷰 (선택)] → [모하나] 대사 / [동물이름] 대사 태그 형식\n"
            "[마무리] → 해결책 없으면 솔직히 인정 → '동물의 문제가 아닌 인간의 문제' 귀결 검토 → 철학적 여운 한 문장 + 구독/좋아요\n\n"
...
def api_generate_step3():
    try:
        ...
        prompt = (
            f"아이템: {item}\n스크립트:\n{script}\n\n"
            "아래 4가지를 한 번에 생성해줘.\n\n"
            "1. title: 유튜브 제목 (30자 이내, 이모지 없이)\n"
            "   → 이 스크립트에서 제작자가 전달하고 싶은 핵심 발견을 제작자 시점으로 한 문장으로\n"
            "2. thumbnail_text: 썸네일 문구 (15자 이내)\n"
            "   → 이 영상을 본 시청자가 주변에 말할 법한 한 마디, 또는 썸네일을 보고 내뱉을 반응을 짧게\n"
